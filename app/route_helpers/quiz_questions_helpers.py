...
def save_attempt(quiz_form, questions):
    """ save the partially completed quiz attempt to the database """
    form_data = quiz_form.data
...
def update_user_stats(score):
    """ update num attempts and score stats for user """
    ...
    if user_average is None:
        ...
        new_average = round((old_average_total + score) / user_stats.num_quiz_attempts, 2)
...
def get_most_recent_attempt_id():
    ...
    return (
        ...
        .order_by(SubmittedAttempt.id.desc())
        .first()
        .id
    )
...
def submit_attempt(quiz_form, questions):
...
def create_quiz_form():
    ...
    return (quiz_form, questions)


def create_quiz():
    """ return (True, redirect obj) if redirected, else (false quiz_form) """
    redirected = False

    # get the quiz Form and the list of questions that will be in the quiz
    quiz_form, questions = create_quiz_form()

    # if the user has chosen an answer for each question
    if quiz_form.validate_on_submit():

        # if the submit button was pressed
        if quiz_form.submit.data:
            # calculate their score for this quiz
            score, attempt_id = submit_attempt(quiz_form, questions)
            ...
            redirected = True
            return (redirected, redirect(url_for('result', score=score, attempt_id=attempt_id)))

    # if the save button was pressed
    if quiz_form.save.data:
        # delete any previously saved attempt
        if current_user.has_saved_attempt:
            delete_saved_attempts()

        # save this attempt
        save_attempt(quiz_form, questions)

        redirected = True
        return (redirected, redirect(url_for('quiz')))

    return (redirected, quiz_form)
